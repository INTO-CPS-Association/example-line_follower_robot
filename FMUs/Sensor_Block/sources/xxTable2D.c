...
static char g_lastError[LASTERRMSGBUFSIZE]={0};
...
void Table2D_Terminate()
{
	...
	int i = 0;
	for (i = 0; i < XXTABLE_FILE_COUNT; ++i)
	{
...
XXInteger Table2D_Table2DInit(XXDouble* inarr, XXInteger inputs, XXDouble* outarr, XXInteger outputs, XXInteger major)
{
	...
	printf("Processing file: '%s' id will be %d\n",fileName,g_table_count);
	...
	printf("Allocated table id %d, ptr %p\n",g_table_count,g_table2dFiles[g_table_count]);
	printf("Allocated table info ptr %p, x_ptr %p, y_ptr %p\n",g_table2dFiles[g_table_count],g_table2dFiles[g_table_count]->xValues,g_table2dFiles[g_table_count]->yValues);
...
XXInteger Table2D_TableRead(XXDouble* inarr, XXInteger inputs, XXDouble* outarr, XXInteger outputs, XXInteger major)
{
	...
	id = (int)inarr[0];
	if (id < 0 || id > XXTABLE_FILE_COUNT) // check for correct id.
	{
	...
	printf("Detected table as id %d and ptr: %p, x_ptr %p, y_ptr %p\n",id,theTable,theTable->xValues, theTable->yValues);
	if (theTable == NULL)
	{
	...
	xValue = inarr[2];
	yValue = inarr[1];
	printf("xValue %lf, yValue %lf\n",xValue,yValue);
	printf("lookup tabke rows %d, cols %d\n",theTable->nRows,theTable->nColumns);
	...
	*columns = 0;
			...
			case EOF:
				if (tmpCols < 2)
				{
					/* Less than two columns found */
						printf("Less than two columns found\n");
					return XXFALSE;
				}
	...
	if (*rows < 2)
	{
		/* Less than two rows found */
			printf("Less than two columns found 2\n");
		return XXFALSE;
	}

	printf("Dimentions before 1-line skip: rows %d, cols %d\n",*rows,*columns);
	/* Ignore the first data point in file (i.e. element of the first row/column). */
	--(*rows);
	--(*columns);
	printf("Dimentions: rows %d, cols %d\n",*rows,*columns);
	return XXTRUE;
}
...
XXInteger findPosition(XXDouble value, XXDouble* array, XXInteger size)
{
	...
	if (size == 0)
		...
		printf("findPosition array[0] = %lf\n",array[0]);

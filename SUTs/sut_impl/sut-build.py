#! /usr/bin/env python
# This script initialises the 'lfr_rtt_mbt/SUT' in RTT_TestProcedure 
...
{					   
	...
	@PROCESS:{
		...
		while(@rttIsRunning){
			...
			sensorLeftVal = rttIOPre->sensorLeftVal;
			sensorRightVal = rttIOPre->sensorRightVal;

			sut_run(sensorLeftVal, sensorRightVal, &servoLeft, &servoRight);

			/* Map SUT output to FMU output: rttIOPost->X = X */
			rttIOPost->servoLeft = servoLeft;
			rttIOPost->servoRight = servoRight;
...
print """
----------------------------------------------------------------------
   Project Initialisation Finished: lfr_ctrl 
----------------------------------------------------------------------
"""
